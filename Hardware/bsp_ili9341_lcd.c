...
{
	* ( __IO uint16_t * ) ( FSMC_Addr_ILI9341_CMD ) = usCmd; //必须要加 __IO 不能让编译器优化到寄存器的缓存中去，而应该在SRAM的内存中访问
                                                             //只要定义了一个值没改变过，就可能被优化
}
...
static void ILI9341_REG_Config ( void )
{
  ...
  if(lcdid == LCDID_ILI9341)
  {
    ...
    /* column address control set 列数*/ 
    ILI9341_Write_Cmd ( CMD_SetCoordinateX ); 
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0x00 ); //0x0000 起点
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0xEF ); //0x00EF = 239 终点
    
    /* page address control set 行数*/
    DEBUG_DELAY ();
    ILI9341_Write_Cmd ( CMD_SetCoordinateY ); 
    ILI9341_Write_Data ( 0x00 );
    ILI9341_Write_Data ( 0x00 ); //0x0000 起点
    ILI9341_Write_Data ( 0x01 );
    ILI9341_Write_Data ( 0x3F ); //0x013F = 319 终点
    
    /*  Pixel Format Set (3Ah)  色深16bit*/
    DEBUG_DELAY ();
...
/*
另外假设打开的图片的实际像素为200*256----生成的数组是102400----那有什么关系呢？
